...
func AddArticleComment(ctx *gin.Context, addCommentReq dto.AddCommentReq) (vo.AddCommentResp, error) {
	...
	// 发送回复通知
	InsertReplyMessage(addCommentReq, comment.ID, userId, global.CONTENT_TYPE_ARTICLE)
	...
	for i := 0; i < length; i++ {
		...
		newAtMsg[i].Type = global.CONTENT_TYPE_ARTICLE
	}
...
func GetArticleComment(ctx *gin.Context, vid uint, page, pageSize int) ([]vo.CommentResp, int64, error) {
	...
	global.Mysql.Model(&model.Comment{}).Where("cid = ? and `type` = ?", vid, global.CONTENT_TYPE_ARTICLE).Count(&total)
	err := global.Mysql.Model(&model.Comment{}).Select(vo.COMMENT_FIELD).
		...
		Where("`comment`.parent_id = 0 and `comment`.deleted_at is null and `comment`.cid = ? and `comment`.`type` = ?", vid, global.CONTENT_TYPE_ARTICLE).
...
// 删除评论回复
func DeleteArticleComment(ctx *gin.Context, id uint) error {
	...
	video, err := FindArticleById(comment.Cid)
	if err != nil {
...
func GetArticleCommentList(ctx *gin.Context, cid uint, page, pageSize int) ([]vo.CommentListResp, int64, error) {
	...
	if cid != 0 {
		...
		db = db.Where("cid = ? and `type` = ?", cid, global.CONTENT_TYPE_ARTICLE)
	} else {
		db = db.Where("cid in (?) and `type` = ?", global.Mysql.Model(&model.Article{}).
			Select("id").Where("uid = ?", userId), global.CONTENT_TYPE_ARTICLE)
	}
